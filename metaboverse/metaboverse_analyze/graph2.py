...
TODO: For some reason, looks like some of the references are incomplete. For example, can't find glutamate in the CHEBI reference
"""
def __main__(
...
network['pathways']['R-HSA-5213460'].keys()


for x in network['pathways'].keys():
    if 'Citric acid cycle (TCA cycle)' in network['pathways'][x]['pathway_name']:
        print('yes')
        for y in network['pathways'][x]['reactions'].keys():

            if 'isocitrate + NAD+' in network['pathways'][x]['reactions'][y]['name']:
                print('hello')
                print(network['pathways'][x]['reactions'][y])


network['master_reference']['R-ALL-5696412']

for x in network['master_reference'].keys():
    if 'glutamate' in network['master_reference'][x]:
        print(network['master_reference'][x])




for x in network['chebi_reference'].keys():
    if 'glut' in network['chebi_reference'][x]['analyte']:
        print(network['chebi_reference'][x])

['R-ALL-5696412']['analyte']
